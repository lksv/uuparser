...
class ChartItem {
  ...
  copyHistoryFrom(other) {
    ...
    other.history.forEach(otherHistory => {
      var otherHistoryHashCode = otherHistory.hashCode();
  ...
  toString() {
    var rhsBeforeDot = this.rule.rhs.slice(0, this.dot);
    var rhsAfterDot  = this.rule.rhs.slice(this.dot);
...
class ChartItemIndex {
   ...
   */
  get(symbol, idx) {
    var res = this.index.get(symbol.code);
    if (res)
      return res.values();
    var map = new Map();
    this.index.set(symbol.code, map);
    return map.values();
  }
   ...
   * @returns {void}
   */
  add(symbol, chartItem) {
    var map = this.index.get(symbol.code);
    if (!this.index.get(symbol)) {
      map = new Map();
      this.index.set(this.hashCode(), map);
    }
...
class Chart {
   ...
   */
  add(chartItem) {
    var alreadyExists = this.get(chartItem);
    if (alreadyExists) {
    ...
    var nextSymbol = chartItem.nextSymbol();
  ...
  get(chartItem) {
     ...
     var addingHashCode = chartItem.hashCode();
